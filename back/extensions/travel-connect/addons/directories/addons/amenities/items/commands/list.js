...
commands.Item({
    id: 'tc.amenities:list',
    exposed: true,
    method: 'GET',
    endpoint: '/api/tc/amenities',
    in: {
        filters: {
            type: 'array',
            each: {
    ...
    callback: async function(properties, resolve)
    {
        ...
        let query = directories.amenities.Find().filter('team_id', user.team.id);
        ...
        resolve({ amenities: items.map(item => item.Get(FIELDS)) });
    }
});
