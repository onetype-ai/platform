...
commands.Item({
    id: 'tc.transportation:list',
    exposed: true,
    method: 'GET',
    endpoint: '/api/tc/transportation',
    in: {
        filters: {
            type: 'array',
            each: {
    ...
    out: {
        transportation: ['array', null, true]
    },
    callback: async function(properties, resolve)
    {
        ...
        let query = directories.transportation.Find().filter('team_id', user.team.id);
        ...
        resolve({ transportation: items.map(item => item.Get(FIELDS)) });
    }
});
